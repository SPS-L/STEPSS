...
- **A STEPSS release is all platforms or none, and the draft is what enforces it.** java-ui's `release` job creates the release with `--draft`, which creates no tag and shows nothing to users. The three `bundles` legs attach their installers to that draft, building the pin **commit** rather than the tag, because the tag does not exist yet. A `promote` job then publishes it and fires both package-manager dispatches (`stepss-apt`, `stepss-scoop`). A plain `needs:` makes `promote` *skip* when any leg fails, and a skipped job leaves a run green, so a `discard` job deletes the draft and exits 1 whenever `needs.bundles.result != 'success'`. A failed release therefore leaves no tag, no release and no half-published set, and re-running reuses the same version number.
